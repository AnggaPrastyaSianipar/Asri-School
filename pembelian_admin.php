...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $query = "SELECT * FROM pembelian_seragam WHERE 
              nama LIKE '%$search%' OR
              jenis_kelamin LIKE '%$search%' OR
              kelas LIKE '%$search%' OR
              seragam LIKE '%$search%' OR
              ukuran LIKE '%$search%' OR
              alamat LIKE '%$search%' OR
              harga LIKE '%$search%' OR
              tanggal_pembelian LIKE '%$search%' OR
              status LIKE '%$search%'";
} else {
    $query = "SELECT * FROM pembelian_seragam";
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'accept') {
        $updateQuery = "UPDATE pembelian_seragam SET status='Accepted' WHERE id=$id";
        mysqli_query($koneksi, $updateQuery);
    } elseif ($action == 'reject') {
        $updateQuery = "UPDATE pembelian_seragam SET status='Rejected' WHERE id=$id";
        mysqli_query($koneksi, $updateQuery);
    } elseif ($action == 'delete') {
        $deleteQuery = "DELETE FROM pembelian_seragam WHERE id=$id";
        mysqli_query($koneksi, $deleteQuery);
    }

    
    header('Location: pembelian_admin.php');
    exit();
}
?>

    <div class="container mt-5">
        <h2>Data Pembelian Seragam</h2>
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        if ($row['status'] == 'Pending') {
                            echo "<form method='POST' action='pembelian_admin.php' class='d-inline'>";
